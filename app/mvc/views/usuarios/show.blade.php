...
<div class="col-md-12">
  <div class="box">
    <div class="box-header with-border">
      ...
      <form action="{{asset('usuarios/editar')}}" method="POST" id="formulario">
        <div class="row" >
          <div class="col-md-6">
            <label>Usuário</label>
            <input type="number" name="id" id="id" value="{{$usuario->id}}" hidden>
            <input class="form-control" required type="text" maxlength="50" placeholder="Usuário" name="usuario" id="usuario" disabled value="{{$usuario->usuario}}">
          </div>
          <div class="col-md-6">
            <label>Email</label>
            <input class="form-control" required type="email" maxlength="200" placeholder="email" name="email" id="email" disabled value="{{$usuario->email}}">
          </div>          
        </div>
        <div class="row">
          <div class="col-md-1">
            <label>Administrador</label>
              <input type="text" value="{{$usuario->admin}}" hidden name="admin" id="admin">
              <div class="slideThree">  
                @if($usuario->admin=='S')
                  <input type="checkbox" disabled checked value="{{$usuario->admin}}" id="admin_checkbox" name="admin_checkbox"/>
                @else
                  <input type="checkbox" disabled value="{{$usuario->admin}}" id="admin_checkbox" name="admin_checkbox"/>
                @endif
